...
if __name__ == "__main__":
    ...
    eef_p = np.array(rt_output_data_list.actual_tcp_position, dtype=float)
    print("eef_p=", eef_p)

    #lin_pos = actual_tcp_position[:3] * 0.001
    #quat_ori = R.from_euler("ZYZ", actual_tcp_position[3:], degrees=True).as_quat()
    #eef_p = np.concatenate([lin_pos, quat_ori])
    eef_ex_force = np.array(rt_output_data_list.external_tcp_force, dtype=float)
    external_torque=api.get_external_torque()
    tool_force=api.get_tool_force()
    joint_torque=api.get_joint_torque()
    print("eef_ex_force", eef_ex_force)
    print("external_torque", external_torque)
    print("joint_torque", joint_torque)
    print("tool_force", tool_force)
    delta_X=0.0
    delta_Y = 0.0
    K1=50
    K2=50
    ...
    assert api.set_safety_mode(SAFETY_MODE.SAFETY_MODE_AUTONOMOUS, SAFETY_MODE_EVENT.SAFETY_MODE_EVENT_MOVE)
    u=time.time()
    while time.time()-u<=40:
        start=time.time()
        rt_output_data_list=api.read_data_rt()
        eef_pos_quat = np.array(rt_output_data_list.actual_tcp_position, dtype=float)
        #lin_pos = actual_tcp_position[:3] * 0.001
        #quat_ori = R.from_euler("ZYZ", actual_tcp_position[3:], degrees=True).as_quat()
        #eef_pos_quat = np.concatenate([lin_pos, quat_ori])
        eef_ex_force = np.array(rt_output_data_list.external_tcp_force, dtype=float)
        #print("type(eef_pos_quat)",type(eef_pos_quat))
        ex_torque = np.linalg.norm(eef_ex_force[0: 2])
        print("ex_torque=", ex_torque)
        delta_X = delta_X + T * ((eef_ex_force[0] - K1 * delta_X) / B1)
        print("delta_X=", delta_X)
        eef_p[0]=eef_pos_quat[0]+(delta_X*1000)
        delta_Y = delta_Y + T * ((eef_ex_force[1] - K2 * delta_Y) / B2)
        print("delta_X=", delta_Y)
        eef_p[1]=eef_pos_quat[1]+(delta_Y*1000)
        print("eef_p=", eef_p)
        target_vel=np.array([300.0,300.0], dtype=float)
        target_acc = np.array([200.0, 200.0], dtype=float)
        eef_p=np.array(eef_p).tolist()
        float_array_type1 = ctypes.c_float * 6
        pos = float_array_type1(*eef_p)
        float_array_type2 = ctypes.c_float * 2
        target_vel = float_array_type2(*target_vel)
        float_array_type3 = ctypes.c_float * 2
        target_acc = float_array_type3(*target_acc)
        api.servol(pos,target_vel,target_acc,1/100)
        time.sleep(1/200)
